...
## Overview ##
A course project for the Coursera Statistical Inference course
...
After taking the mean of 1000 draws of 40 observations from the exponential distribution, the average mean is `r round(mean(ExponentialMeanSimulations), 2)`, compared to the theoretical mean of `r TheoreticalMean`. Our sample mean is now much closer to the theoretical mean.
...
## Distribution ##
This exercise provides some insight into how the CLT works in practise. As we increase sample size, for example from 40 to 1000, we can expect the sample distribution to approach the theoretical exponential distribution. For example, consider the image below with N=40 and N=1000.
...
Now, consider the distribution of the average of the sample mean and variance as simulated in part one of this document below. A blue reference line displaying the standard normal density is used to show that the distribution of sample means and variances are approximately normal.
